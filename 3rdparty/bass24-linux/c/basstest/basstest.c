...
void AddClicked(GtkButton *obj, gpointer data)
{
	...
	if (resp == GTK_RESPONSE_ACCEPT) {
		...
		switch (mode) {
			case 1: // add a stream
				{
					HSTREAM handle = BASS_StreamCreateFile(0, file, 0, 0, BASS_SAMPLE_FLOAT);
					if (handle)
			...
			case 2: // add a MOD music
				{
					HMUSIC handle = BASS_MusicLoad(0, file, 0, 0, BASS_MUSIC_RAMP | BASS_SAMPLE_FLOAT, 1);
					if (handle)
...
void PlayClicked(GtkButton *obj, gpointer data)
{
	...
	if (mode == 3) { // sample: get a channel and set volume=50% and pan=random
		handle = BASS_SampleGetChannel(handle, 0);
...
{
	double value = gtk_range_get_value(range);
	BASS_SetVolume(value / 100); // output volume (0-1)
}
...
{
	BASS_SetConfig(BASS_CONFIG_UPDATETHREADS, gtk_toggle_button_get_active(obj) ? 2 : 1); // set 1 or 2 update threads
}
