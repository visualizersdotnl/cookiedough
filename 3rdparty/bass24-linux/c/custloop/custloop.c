/*
	...
	Copyright (c) 2004-2024 Un4seen Developments Ltd.
*/
...
BOOL PlayFile()
{
	...
	if (gtk_dialog_run(GTK_DIALOG(filesel)) == GTK_RESPONSE_ACCEPT) {
		...
		if (!(chan = BASS_StreamCreateFile(0, file, 0, 0, BASS_SAMPLE_LOOP))
			&& !(chan = BASS_MusicLoad(0, file, 0, 0, BASS_MUSIC_RAMP | BASS_MUSIC_POSRESET | BASS_MUSIC_PRESCAN | BASS_SAMPLE_LOOP, 1))) {
...
void DrawTimeLine(cairo_t *cr, QWORD pos, float r, float g, float b, DWORD y)
{
	cairo_text_extents_t extends;
	DWORD wpos = pos / bpp, time;
	char text[16];

	cairo_set_source_rgb(cr, r, g, b);
	cairo_set_line_width(cr, 1);
	cairo_move_to(cr, wpos, 0);
	cairo_line_to(cr, wpos, HEIGHT);
	cairo_stroke(cr);
	
	time = BASS_ChannelBytes2Seconds(chan, pos) * 1000; // position in milliseconds
	sprintf(text, "%u:%02u.%03u", time / 60000, (time / 1000) % 60, time % 1000);
	cairo_text_extents(cr, text, &extends);
	y += extends.height + 1;
	if (wpos >= WIDTH / 2)
		wpos -= extends.width + 1;
	else
		wpos++;
	cairo_set_font_size(cr, 11);
	cairo_move_to(cr, wpos, y);
	cairo_show_text(cr, text);
}

gboolean DrawDisplay(GtkWidget *obj, cairo_t *cr, gpointer data)
{
	cairo_save(cr);

	gdk_cairo_set_source_pixbuf(cr, wavepb, 0, 0);
	cairo_paint(cr);
	DrawTimeLine(cr, loop[0], 0, 1, 1, 12); // loop start
	if (loop[1]) DrawTimeLine(cr, loop[1], 1, 1, 0, 24); // loop end
	DrawTimeLine(cr, BASS_ChannelGetPosition(chan, BASS_POS_BYTE), 1, 1, 1, 0); // current position
...
int main(int argc, char* argv[])
{
	...
	g_signal_connect(win, "destroy", G_CALLBACK(WindowDestroy), NULL);

	GtkWidget *ebox = gtk_event_box_new();
	gtk_container_add(GTK_CONTAINER(win), ebox);
	g_signal_connect(ebox, "button-release-event", G_CALLBACK(WindowButtonRelease), NULL);

	displayarea = gtk_drawing_area_new();
	gtk_widget_set_size_request(displayarea, WIDTH, HEIGHT);
	g_signal_connect(displayarea , "draw", G_CALLBACK(DrawDisplay), NULL);
	gtk_container_add(GTK_CONTAINER(ebox), displayarea);
