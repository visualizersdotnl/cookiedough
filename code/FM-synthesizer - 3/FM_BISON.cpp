...
namespace SFM
{
	...
	*/
	...
	SFM_INLINE float CalcOpIndex(bool isCarrier, unsigned key, float velocity, const FM_Patch::Operator &patchOp)
	{
		...
		if (false == isCarrier)
		{
			// If it looks like a hack it probably is; I'm using the DX envelope to output level mapping here to convert
			// amplitude to a modulation depth/index, which is in no shape or form correct but *does* give that nice Yamaha twist
			// to things
			const unsigned index = 128+unsigned(output*99.f);
...
//		voice.Reset();
	...
	*/
	...
	/*
		...
		Figure 14 shows a triple-path configuration in which the signals generated by the first and second delays are mixed and sent to the left output, 
		while the signals generated by the second and third delays are mixed and sent to the right. In this scheme, you can leave out the 'straight-through' signal, 
		because the dual inputs to each mixer will be chorusing differently, and — far from contributing to the result — the original might actually damage the impression of width and depth.
