...
//		SFM_ASSERT(pitchBend >= -1.f && pitchBend <= 1.f);
		...
		{
		case kSingle:
			sample = m_carrierA.Sample(sampleCount, modulation, m_pulseWidth);
			break;

		case kDoubleCarriers:
			sample = fast_tanhf(m_carrierA.Sample(sampleCount, modulation, m_pulseWidth) + m_carrierB.Sample(sampleCount, -modulation, m_pulseWidth));
			break;
		}
