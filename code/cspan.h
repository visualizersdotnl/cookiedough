...
// copy of cspan(), takes pre-unpacked (16-bit) colors as __m128i
...
// copy of cspan(), takes pre-unpacked (32-bit) colors as __m128i
VIZ_INLINE void cspanISSE32(
	uint32_t *pDest,
	int destIncr, 
	unsigned int length,     // length of span
	unsigned int drawLength, // visible length (for correct interpolation)
	__m128i A, __m128i B)    // colors, unpacked to 32-bit
{
	VIZ_ASSERT(drawLength > 0 && length >= drawLength);
